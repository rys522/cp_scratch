...
SAFE_THRESHOLD = ROBOT_RAD + OBSTACLE_RAD + 1.0

# Termination Settings
GOAL_REACH_THRESHOLD = 2.0 # Stop if within 2.0 units of the goal
...
def build_cv_predictions(trajs_true, box, horizon):
    ...
    for t in range(horizon + 1, T):
        base = t - horizon
        y_base, y_prev = trajs_true[:, base], trajs_true[:, base - 1]
        v = y_base - y_prev
        curr = y_base.copy()
...
def run_eth_cp_mpc_visual():
    ...
    train_true_raw, _, train_mask, test_true_raw, _, test_mask = load_eth_pickle_preprocessed(
        dataset=DATASET, box=BOX, T=TSTEPS, split_ratio=0.8, seed=SEED, base_dir=BASE_DIR, num_peds=NUM_PEDS
    )
    ...
    ctrl = CPFieldMPCController(
        ...
        robot_rad=ROBOT_RAD, obstacle_rad=OBSTACLE_RAD,
    ...
    extent = [xs[0] + WORLD_CENTER[0], xs[-1] + WORLD_CENTER[0], 
    ...
    axs[1].set_title(f"Safety Bounds (Alpha={ALPHA}): CP (Red) vs True (Blue)")
    ...
    for ax in axs:
        ...
        plan, = ax.plot([], [], "c--", lw=1.5, zorder=4)
        peds = ax.scatter([], [], c="red", s=25, edgecolors="black", zorder=6)
        robot_dots.append(dot); traj_lines.append(line); plan_lines.append(plan); peds_scatters.append(peds)

    status_text = fig.text(0.5, 0.02, "", ha="center", fontsize=12, color="black", fontweight="bold")
    collision_count = 0

    def update(k):
        nonlocal robot_xy, robot_th, collision_count
        t = k + 1
        
        # Check Goal condition
        dist_to_goal = np.linalg.norm(robot_xy - goal)
        if dist_to_goal < GOAL_REACH_THRESHOLD:
            status_text.set_text(f"SUCCESS: Goal Reached! Distance: {dist_to_goal:.2f}")
        ...
        true_mask = (true_dist < SAFE_THRESHOLD).astype(float)
        cp_mask = (lower_field < SAFE_THRESHOLD).astype(float)
        ...
        v, w = (float(act[0]), float(act[1])) if (act is not None and info.get("feasible")) else (0.0, 0.0)
        ...
        p_show = p_t[vis]
        dmin = min_dist_robot_to_peds(robot_xy, p_show)
        is_coll = (dmin < (ROBOT_RAD + OBSTACLE_RAD))
        if is_coll: collision_count += 1

        im_heat.set_data(lower_field)
        im_true.set_data(true_mask)
        im_cp.set_data(cp_mask)
        ...
        for i in range(2):
            ...
            peds_scatters[i].set_offsets(p_show if p_show.size else np.zeros((0, 2)))

        # Update Status Text with Actions (v, w)
        status_text.set_text(
            f"Step: {t} | Collisions: {collision_count} | dmin: {dmin:.2f} | "
            f"Action: v={v:.2f}, w={w:.2f} | Goal Dist: {dist_to_goal:.2f}"
        )
        if is_coll: status_text.set_color("red")
        else: status_text.set_color("black")
